...
def buildDockerImage(cleanBuild):
    if cleanBuild:
        ...
        os.system('docker builder build --platform linux/arm64 --force-rm=true -t pacman:latest -f ./docker/Dockerfile . --no-cache')
    else:
        print("\n \t[+] Semi-building using older docker image.\n")
        os.system('docker builder build --platform linux/arm64 --force-rm=true -t pacman:latest -f ./docker/Dockerfile .')
